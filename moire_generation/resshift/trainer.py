...
class TrainerBase:
    ...
    def init_logger(self):
        print('self.configs.resume', self.configs.resume)
...
class MoireTrainer(TrainerBase):
    def setup_optimizaton(self):
        super().setup_optimizaton() # optim, scaler
        if self.configs.train.lr_schedule == 'cosin':
            self.lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                optimizer=self.optimizer,
                T_max=self.configs.train.iterations - self.configs.train.warmup_iterations,
    ...
    def train(self):
        ...
        # close the tensorboard
        self.close_logger()
    # degradtion 과정은 필요없어서 _dequeue_and_enqueue, prepare_data 생략
    def backward_step(self, dif_loss_wrapper, micro_data, num_grad_accumulate, tt):
        context = torch.cuda.amp.autocast if self.configs.train.use_amp else nullcontext
        with context():
            losses, z_t, z0_pred = dif_loss_wrapper()
    ...
    def training_step(self, data):
        current_batchsize = data['gt'].shape[0] # 64
        micro_batchsize = self.configs.train.microbatch
        num_grad_accumulate = math.ceil(current_batchsize / micro_batchsize) # 8
        ...
        for jj in range(0, current_batchsize, micro_batchsize):
            ...
            latent_downsamping_sf = 2**(len(self.configs.autoencoder.params.ddconfig.ch_mult) - 1) # 4 [1,2,4] => (3-1)**2 = 4
            latent_resolution = micro_data['gt'].shape[-1] // latent_downsamping_sf # 256 / 4 = 64 
            if 'autoencoder' in self.configs:
                noise_chn = self.configs.autoencoder.params.embed_dim # 3
            else:
                noise_chn = micro_data['gt'].shape[1]
            noise = torch.randn(
                size= (micro_data['gt'].shape[0], noise_chn,) + (latent_resolution, ) * 2,
            ...
            if self.configs.model.params.cond_lq: # 이건 뭘까
                model_kwargs = {'lq':micro_data['lq'],}
                if 'mask' in micro_data:
                    model_kwargs['mask'] = micro_data['mask']
            else:
                model_kwargs = None
            # cond_lq 이걸하면 model_kwargs : {'lq' : [mb, 3, 64, 64]}
            compute_losses = functools.partial(
                self.base_diffusion.training_losses,
                self.model,
                micro_data['gt'],
...
class MoireTrainerLPIPS(MoireTrainer):
    def backward_step(self, dif_loss_wrapper, micro_data, num_grad_accumulate, tt):
        loss_coef = self.configs.train.get('loss_coef')
        context = torch.cuda.amp.autocast if self.configs.train.use_amp else nullcontext
        # diffusion loss
