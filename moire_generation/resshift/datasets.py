...
def create_dataset(dataset_config):
    ...
    elif dataset_config['type'] == 'uhdm':
        dataset = UHDMDataSet(**dataset_config['params'])
    else:
...
class UHDMDataSet(Dataset):
    def __init__(self, 
                dir_path='/purestorage/project/hkl/FAS_DnC/data/webdata/uhdm_data/train',
                transform_type='uhdm',
                img_size=256
                ):
        super().__init__()
        self.file_paths = _list_image_files_recursively(dir_path)
    ...
    def __getitem__(self, index):
        ...
        im_path = self.file_paths[index]
        moire_path = os.path.split(im_path)[0] + '/' + os.path.split(im_path)[-1][0:4] + '_moire.jpg'
        
        clean_img = pil_rgb_convert(exif_transpose(Image.open(im_path)))
        moire_img = pil_rgb_convert(exif_transpose(Image.open(moire_path)))
        clean_img, moire_img = self.transformation((clean_img, moire_img))
        
        out_dict = {'lq':clean_img, 'gt':moire_img}
        return out_dict