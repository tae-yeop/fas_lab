...
class GaussianDiffusion:
    ...
    def q_sample(self, x_start, y, t, noise=None):
        ...
        # self.etas : [4,]
        # t : 랜덤하게 뽑은 타임인덱스
        # _extract_into_tensor(self.etas, t, x_start.shape) : [8, 3, 64, 64]
        # 각 timestep에 맞는 etas를 뽑아내고 싶을 때 사용
        return (
    ...
    def p_sample_loop_progressive(
        ...
        # upsample =False이면 [8, 3, 64, 64]
        # generating noise
        if noise is None:
            noise = th.randn_like(z_y) # [8, 3, 256, 256]
        if noise_repeat:
            noise = noise[0,].repeat(z_y.shape[0], 1, 1, 1)
        z_sample = self.prior_sample(z_y, noise) # [8, 3, 256, 256]
        ...
        # indices : [3,2,1,0]
        for i in indices:
            t = th.tensor([i] * y.shape[0], device=device) # [9]
            with th.no_grad():
                out = self.p_sample(
                    model, #Unetswin
                    z_sample,
    ...
    def training_losses(
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.WEIGHTED_MSE:
            # 모델에 들어가는 인풋  : [8, 3, 64, 64], lq : [8, 3, 256, 256]
