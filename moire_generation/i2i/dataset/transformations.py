...
class AlbumentationsTransform:
    def __init__(self):
        # image1은 기본 타겟, image2는 추가 타겟으로 설정
    ...
    def __call__(self, img_pair):
        # Albumentations는 이미지를 HxWxC 형태의 NumPy 배열로 요구합니다.
        # PIL 이미지를 NumPy 배열로 변환
        img1 = np.array(img_pair[0])
        img2 = np.array(img_pair[1])
        
        # 변환 적용
        transformed = self.transform(image=img1, image2=img2)
        transformed_img1 = transformed['image']
        transformed_img2 = transformed['image2']
        
        # ToTensorV2 변환은 여기에서 별도로 적용
        return ToTensorV2()(image=transformed_img1)['image'], ToTensorV2()(image=transformed_img2)['image']