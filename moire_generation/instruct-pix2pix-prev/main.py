...
class CUDACallback(Callback):
    ...
    def on_train_epoch_start(self, trainer, pl_module):
        root_gpu_index = trainer.strategy.root_device.index
        # Reset the memory use counter
        # torch.cuda.reset_peak_memory_stats(trainer.root_gpu)
        # torch.cuda.synchronize(trainer.root_gpu)
        torch.cuda.reset_peak_memory_stats(root_gpu_index)
        torch.cuda.synchronize(root_gpu_index)
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        ...
        torch.cuda.synchronize(root_gpu_index)
        max_memory = torch.cuda.max_memory_allocated(root_gpu_index) / 2 ** 20
...
if __name__ == "__main__":
    ...
    try:
        ...
        trainer = Trainer.from_argparse_args(trainer_opt, strategy=DDPStrategy(find_unused_parameters=False), accelerator='gpu', devices=1, **trainer_kwargs)
        ...
        if not cpu:
            #  lightning_config.trainer.gpus = int
